...
//!
//! Delivery-id replay dedup (GitHub's HMAC scheme carries no timestamp or
//! nonce of its own) lives in `ignite-db-store`'s `record_webhook_delivery_once`
//! instead of in-process state here — persisted so a server restart can't
//! reopen the replay window for everything received before it.
