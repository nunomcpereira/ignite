...
/// `body` using `secret` — same implementation as `code_scanning_webhook.rs`/
/// `push_protection_webhook.rs`'s own copies (no shared "webhook utils"
/// crate; kept duplicated deliberately, see those files' own comments).
