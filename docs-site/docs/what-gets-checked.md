...
| A secret that was committed, then removed in a later commit — still exposed to anyone who clones the repo, even though it's gone from the current working tree | Optional supplemental pass: runs `gitleaks detect` against full commit history instead of just the working tree (`security.gitleaks.scanHistory`, off by default) | gitleaks (history mode) | Skipped — no fallback; requires a real `.git` directory (a no-op on an uploaded ZIP with no git history) |
...
| **Env-var documentation drift** | Compares the environment variables your code reads by literal name (`env::var("X")`, `process.env.X`, `os.getenv("X")`, `os.Getenv("X")`, `System.getenv("X")`, docker-compose `${X}`, ...) against `.env.example`/`.env.sample`/`.env.template`. A read the template doesn't list is flagged at its first read site; a template entry nothing mentions is flagged as a stale entry. Commented-out `# X=` lines count as documented. OS/CI variables (`HOME`, `PATH`, `CARGO_*`, `GITHUB_*`, ...) and test code are ignored, as are dynamic names like `env::var(name)`. Always advisory (category `config-drift`); a project with no template reports the check as not applicable | On by default — `ENV_VAR_DRIFT_ENABLED=false` to opt out |
