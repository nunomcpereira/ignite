...
ID: semantic-sast::lib/tool-runner.js::209
# [ERROR] semantic-sast - Detected calls to child_process from a function argument `binaries`. This could lead to a command injection if the input is user controllable. Try to avoid calls to child_process, and if it is needed ensure user input is correctly sanitized or sandboxed. 
#   lib/tool-runner.js:209
# Code: child = spawn(binaries.codeql, safeArgs, { cwd: safeCwd, env: safeEnv });
Acknowledge: Same finding as the already-acknowledged semantic-sast::lib/tool-runner.js::203 - line drifted 203 -> 209 after adding a nosemgrep suppression comment (for the org governance CI's own Semgrep pass, which doesn't share Phase 4's override list) directly above this line. Same code, same reasoning: `binaries` is the object createToolRunner(binaries) is constructed with once at server startup from CONFIG.security.*.binary (config.json/env, operator-controlled), never from a request; safeArgs is an array (no shell:true), already validated by sanitizeCliArgs.

ID: pii-dataflow::lib/tool-runner.js::209
# [ERROR] pii-dataflow - Unsanitized dynamic input in OS command
#   lib/tool-runner.js:209
# Code: child = spawn(binaries.codeql, safeArgs, { cwd: safeCwd, env: safeEnv });
Acknowledge: Same finding as the already-acknowledged pii-dataflow::lib/tool-runner.js::203 - line drifted 203 -> 209 after adding a nosemgrep suppression comment directly above this line (see semantic-sast::lib/tool-runner.js::209 above). Same code, same reasoning: binaries.codeql comes from CONFIG.security.codeql.binary (operator config/env, set once at process startup), never from a request; safeArgs is an array (no shell:true), already validated by sanitizeCliArgs.
