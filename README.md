...
| [Semgrep](https://semgrep.dev) (OSS) | Semantic pattern-matching SAST - logical flaws and injection-style sinks beyond the LLM deep-scan. Ruleset defaults to `p/security-audit,p/owasp-top-ten,ignite-auth-correctness-rules.yaml` (comma-separated - each pack is passed as its own `--config`, so packs combine rather than replace each other). The last is a small bundled local ruleset (below) for JWT algorithm-confusion, a real vulnerability class the registry packs don't catch. **On by default.** No built-in fallback (semantic rule engines can't be meaningfully approximated). **Known limitation:** Semgrep OSS's taint engine is intraprocedural (single-file) - it can't trace a vulnerability whose tainted input crosses multiple files before reaching a sink (e.g. a stored-XSS/IDOR chain spanning a controller, service layer, and template). That gap is covered only by the LLM deep-scan's best-effort read of the real data flow, not a deterministic guarantee - true cross-file taint analysis needs a paid Semgrep Pro subscription or CodeQL/GitHub Advanced Security, neither of which Ignite wires in. | `brew install semgrep` |
| **Auth-correctness rules** (`ignite-auth-correctness-rules.yaml`) - reuses Semgrep | Two real-vulnerability rules p/security-audit doesn't cover, verified against fixtures before shipping: `jwt.verify()`/`jwt.decode()` called with no explicit `algorithms` allowlist (JS `jsonwebtoken` + Python `PyJWT`) - vulnerable to algorithm-confusion (an RS256-signed token replayed as HS256 using the public key as the HMAC secret). A third rule (session-fixation - session data written post-login without first calling `req.session.regenerate()`) was drafted and dropped: it false-positived on the textbook-correct idiom (`regenerate(callback)` with the write *inside* the callback), since Semgrep OSS's `pattern-not` can't see across that nested-function boundary - shipping a rule that flags safe code as vulnerable would be worse than not having the check. | Same as Semgrep above |
...
| [CodeQL](https://github.com/github/codeql-cli-binaries) | **Cross-file** static analysis (JS/TS, Python, Java, Go by default) - the one check that traces a vulnerability across file/function boundaries, which Semgrep OSS's intraprocedural engine structurally can't do (see the Semgrep row above). Builds a real per-language CodeQL database and runs the `security-extended` query suite. **Off by default** and, unlike every other tool in this table, **never runs on the fast interactive push-time pipeline** regardless of the toggle - only the separate Deep Scan endpoint/scheduler invokes it, since a database build takes minutes per language. Deep-scan results are cached per `(org, repo, language)` keyed by a content hash of that language's file set, so an unchanged codebase skips the rebuild on the next scheduled run. No built-in fallback. Set `CODEQL_ENABLED=true` to opt in. | `scripts/install-tools.sh` or see the CLI binaries repo |
...
- `test/codeql-scan.test.js` - CodeQL cross-file static analysis (deep scan only; real end-to-end case self-skips if `codeql` isn't installed)
