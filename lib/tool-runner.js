 ...
 *   picklescan, oasdiff } — each a binary name/path string. git/gh/act/docker aren't configurable,
 *   so they aren't parameters here.
 */
function createToolRunner(binaries) {
  const ALLOWED_COMMANDS = Object.freeze(new Set(['git', 'gh', 'act', 'docker', 'gitleaks', 'licensee', 'ort', 'trivy', 'checkov', 'hadolint', 'syft', 'cosign', 'semgrep', 'bearer', 'jscpd', 'gocloc', 'spectral', 'guarddog', 'codeql', 'picklescan', 'oasdiff']));
  ...
  function runTool(tool, args, cwd, { env: envOverride = {}, allowedExitCodes = [0], timeoutMs = 120_000 } = {}) {
    return new Promise((resolve, reject) => {
      ...
      switch (safeTool) {
        ...
        case 'picklescan': return execute(binaries.picklescan);
        case 'oasdiff': return execute(binaries.oasdiff);
