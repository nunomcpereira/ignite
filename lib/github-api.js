...
function createGithubApi({ runTool, runToolStreaming }) {
  ...
  // (it rejects \n in any argument — see sanitizeCliArgs), so the `gh` path
  // writes it to a temp file and uses --body-file instead of --body. PR
  // comments are just issue comments under GitHub's REST model, hence the
  // /issues/{number}/comments path for the fallback.
  async function ghCommentOnPr({ fullName, prNumber, body, token }) {
    if (await isGhCliAvailable()) {
      const fs = require('fs/promises');
      const path = require('path');
      // mkdtemp (not a hand-built Date.now()/Math.random() path) so the
      // containing directory name itself is unpredictable — closes the
      // CodeQL insecure-temp-file finding a predictable path triggers even
      // with an exclusive-create write flag.
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ignite-pr-comment-'));
      const tmpFile = path.join(tmpDir, 'body.md');
      await fs.writeFile(tmpFile, body, 'utf8');
      try {
        await runTool('gh', ['pr', 'comment', String(prNumber), '--repo', fullName, '--body-file', tmpFile], os.tmpdir(), { env: { GH_TOKEN: token } });
      } finally {
        await fs.rm(tmpDir, { recursive: true, force: true });
      }
      return;
    }
    await githubApiRequest(token, 'POST', `/repos/${fullName}/issues/${prNumber}/comments`, { body: { body } });
  }
